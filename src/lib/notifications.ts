...
export function buildNotifications(data: AppData, session: SessionUser | null) {
  ...
  achievementSteps.filter((step) => completedEvents >= step).forEach((step) => {
    notifications.push({
  ...
  if (session.role === "admin") {
    data.requests.filter((request) => request.status === "pending").forEach((request) => notifications.push({
      body: `${request.contactName} hat eine neue Veranstaltung angefragt.`,
    ...
    data.registrationRequests.filter((request) => request.status === "pending").forEach((request) => notifications.push({
      body: `${request.name} wartet auf die Freischaltung.`,
      date: request.createdAt,
      href: "/team",
    ...
    data.knowledgeSuggestions.filter((suggestion) => suggestion.pageId === "rules").forEach((suggestion) => notifications.push({
      body: `${suggestion.authorName} hat eine Änderung vorgeschlagen.`,
      date: suggestion.createdAt,
      href: "/rules",
      id: `rules-suggestion:${suggestion.id}`,
      kind: "admin",
      title: "Neuer Regelvorschlag"
    }));
    data.events
      .filter((event) => {
        const startsAt = new Date(event.startsAt).getTime();
      ...
      .forEach((event) => notifications.push({
        body: `${formatEventDate(event.startsAt)} · Es fehlt noch eine Einteilung.`,
        date: event.createdAt,
        href: "/calendar",
        id: `unstaffed:${event.id}`,
        kind: "attention",
        title: event.title
      }));
  }
...
function isStaffed(eventId: string, eventType: string, data: AppData) {
  ...
  if (eventType.trim().toLowerCase() === "termin") {
    return roles.includes("Teilnehmer");
  }
  if (eventType.trim().toLowerCase() === "schulische veranstaltung") {
    return roles.includes("Ton");
  }
