...
import {
  ...
  Image as ImageIcon, Info, LoaderCircle, Menu, MessageCircle, Mic, MoreHorizontal,
    ...
    <RouteGuard><AppShell title="Chat" eyebrow="Team" contentClassName="chat-page">
      <div className="chat-workspace">
        <aside className={chatListOpen ? "chat-list is-open" : "chat-list"}>
          <header><div><strong>Chats</strong><span>{conversations.reduce((sum, item) => sum + item.unreadCount, 0)} ungelesen</span></div><button className="icon-button primary" type="button" aria-label="Neuen Chat starten" onClick={() => setCreateOpen(true)}><Plus size={18} /></button></header>
        ...
        <section className={dragActive ? "team-chat is-dragging" : "team-chat"} aria-label={activeConversation?.name ?? "Chat"} onDragEnter={(event) => { event.preventDefault(); setDragActive(true); }} onDragOver={(event) => { event.preventDefault(); event.dataTransfer.dropEffect = "copy"; setDragActive(true); }} onDragLeave={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node | null)) setDragActive(false); }} onDrop={(event) => { event.preventDefault(); setDragActive(false); void addFiles(event.dataTransfer.files); }}>
          ...
          <div className="chat-stream" onClick={() => { setOpenReaders(null); setContextMenu(null); }}>
            ...
            {messages.map((message) => {
              ...
              return <div className={own ? "chat-message-row is-own" : "chat-message-row"} id={`chat-message-${message.id}`} key={message.id} onPointerDown={(event) => { swipeStartRef.current = { id: message.id, x: event.clientX, y: event.clientY }; }} onPointerUp={(event) => { const start = swipeStartRef.current; swipeStartRef.current = null; if (start?.id === message.id && event.clientX - start.x > 72 && Math.abs(event.clientY - start.y) < 45) { setReplyingTo(message); textareaRef.current?.focus(); } }}>
                <article className={["chat-message", own ? "is-own" : "", emojiOnly ? "is-emoji-only" : "", message.poll ? "has-poll" : "", message.attachments.some((file) => file.type.startsWith("image/")) ? "has-image" : ""].filter(Boolean).join(" ")} onContextMenu={(event) => { event.preventDefault(); event.stopPropagation(); setContextMenu({ message, x: Math.min(event.clientX, window.innerWidth - 230), y: Math.min(event.clientY, window.innerHeight - 330) }); }}>
                  {!emojiOnly ? <div className="chat-message-meta"><strong>{author?.name ?? "Ehemaliges Mitglied"}</strong><time>{formatMessageTime(message.createdAt)}</time>{message.editedAt ? <small>bearbeitet</small> : null}{message.pinnedAt ? <Pin size={11} /> : null}</div> : null}
                  ...
                  {message.poll ? <ChatPollCard message={message} sessionId={session?.id} onVote={(optionId) => void togglePollVote(message, optionId)} /> : message.body ? renderMessageBody(message.body, data.events, data.profiles) : null}
                ...
                {readers.length ? <ChatReaderAvatars readers={readers} open={openReaders === message.id} onToggle={(event) => { event.stopPropagation(); setOpenReaders((current) => current === message.id ? null : message.id); }} /> : null}
              </div>;
            })}<div ref={endRef} />
          </div>
          <div className="chat-composer">
            ...
            <div className="chat-compose-row"><div className="chat-compose-tools"><label className="icon-button chat-tool-button" title="Datei anhängen"><Paperclip size={18} /><input className="visually-hidden" type="file" accept="image/*,video/*,audio/*,.gif,.webp,.pdf,.doc,.docx,.ppt,.pptx,.xls,.xlsx,.zip" multiple onChange={(event) => { if (event.currentTarget.files) void addFiles(event.currentTarget.files); event.currentTarget.value = ""; }} /></label><label className="icon-button chat-tool-button" title="Foto aufnehmen"><Camera size={18} /><input className="visually-hidden" type="file" accept="image/*" capture="environment" onChange={(event) => { if (event.currentTarget.files) void addFiles(event.currentTarget.files); event.currentTarget.value = ""; }} /></label><button className={recording ? "icon-button chat-tool-button is-recording" : "icon-button chat-tool-button"} type="button" aria-label={recording ? "Aufnahme beenden" : "Sprachnachricht aufnehmen"} onClick={() => void toggleRecording()}>{recording ? <Square size={15} fill="currentColor" /> : <Mic size={18} />}</button><button className="icon-button chat-tool-button" type="button" aria-label="Umfrage erstellen" onClick={() => setPollOpen((open) => !open)}><BarChart3 size={18} /></button></div><textarea ref={textareaRef} value={body} onChange={(event) => setBody(event.target.value)} placeholder="Nachricht schreiben…  @Person  [[Veranstaltung]]" rows={1} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); void submit(); } }} /><button className="icon-button primary chat-send-button" type="button" aria-label="Nachricht senden" disabled={sending || (!body.trim() && !attachments.length)} onClick={() => void submit()}>{sending ? <LoaderCircle className="spin" size={19} /> : <Send size={19} />}</button></div>
...
function CreateChatModal({ profiles, sessionId, conversations, onClose, onCreated }: { profiles: Profile[]; sessionId: string; conversations: ChatConversation[]; onClose: () => void; onCreated: (id: string) => void }) {
  const [kind, setKind] = useState<"direct" | "group">("direct"); const [name, setName] = useState(""); const [description, setDescription] = useState(""); const [memberIds, setMemberIds] = useState<string[]>([]); const [saving, setSaving] = useState(false); const [error, setError] = useState("");
  async function create() { const members = memberIds.filter((id) => id !== sessionId); if (!members.length || (kind === "group" && !name.trim())) return; if (kind === "direct") { const existing = conversations.find((conversation) => conversation.kind === "direct" && conversation.memberIds.length === 2 && conversation.memberIds.includes(sessionId) && conversation.memberIds.includes(members[0])); if (existing) { onCreated(existing.id); return; } } setSaving(true); try { const other = profiles.find((profile) => profile.id === members[0]); const id = await createChatConversation(sessionId, kind === "direct" ? other?.name ?? "Direktchat" : name, description, kind, kind === "direct" ? members.slice(0, 1) : members); onCreated(id); } catch (caught) { setError(caught instanceof Error ? caught.message : "Chat konnte nicht erstellt werden."); } finally { setSaving(false); } }
  return <div className="chat-modal-backdrop" onClick={onClose}><section className="chat-create-modal" role="dialog" aria-modal="true" onClick={(event) => event.stopPropagation()}><header><div><MessageCircle size={19} /><strong>Neuer Chat</strong></div><button type="button" onClick={onClose}><X size={19} /></button></header><div className="chat-kind-tabs"><button className={kind === "direct" ? "is-active" : ""} type="button" onClick={() => { setKind("direct"); setMemberIds([]); }}>Einzelchat</button><button className={kind === "group" ? "is-active" : ""} type="button" onClick={() => setKind("group")}>Gruppe</button></div><div className="chat-create-content">{kind === "group" ? <><label><span>Gruppenname</span><input value={name} onChange={(event) => setName(event.target.value)} placeholder="z. B. Bühnenbau" /></label><label><span>Beschreibung</span><textarea value={description} onChange={(event) => setDescription(event.target.value)} rows={2} /></label></> : null}<div className="chat-members"><strong>{kind === "direct" ? "Person auswählen" : "Teilnehmer auswählen"}</strong>{profiles.filter((profile) => profile.id !== sessionId).map((profile) => <label key={profile.id}><ProfileAvatar profile={profile} /><span><strong>{profile.name}</strong><small>{profile.email}</small></span><input type={kind === "direct" ? "radio" : "checkbox"} name="chat-member" checked={memberIds.includes(profile.id)} onChange={(event) => setMemberIds((current) => kind === "direct" ? [profile.id] : event.target.checked ? [...current, profile.id] : current.filter((id) => id !== profile.id))} /></label>)}</div>{error ? <p className="error-text">{error}</p> : null}</div><footer><button className="button primary" type="button" disabled={saving || !memberIds.length || (kind === "group" && !name.trim())} onClick={() => void create()}>{saving ? <LoaderCircle className="spin" size={16} /> : <Plus size={16} />}Chat starten</button></footer></section></div>;
}
...
function ChatPollCard({ message, sessionId, onVote }: { message: ChatMessage; sessionId?: string; onVote: (optionId: string) => void }) { const poll = message.poll; if (!poll) return null; const totalVotes = poll.options.reduce((sum, option) => sum + option.voterIds.length, 0); const participantCount = new Set(poll.options.flatMap((option) => option.voterIds)).size; return <div className="chat-poll"><strong>{poll.question}</strong><div>{poll.options.map((option) => { const selected = Boolean(sessionId && option.voterIds.includes(sessionId)); const percent = totalVotes ? Math.round(option.voterIds.length / totalVotes * 100) : 0; return <button className={selected ? "is-selected" : ""} type="button" key={option.id} onClick={() => onVote(option.id)}><span className="chat-poll-fill" style={{ width: `${percent}%` }} /><span className="chat-poll-choice"><i>{selected ? "✓" : ""}</i><b>{option.label}</b><small>{option.voterIds.length}</small></span></button>; })}</div><small>{participantCount} {participantCount === 1 ? "Stimme" : "Stimmen"}{poll.allowMultiple ? " · Mehrfachauswahl" : ""}</small></div>; }
function ChatReaderAvatars({ readers, open, onToggle }: { readers: Profile[]; open: boolean; onToggle: (event: React.MouseEvent<HTMLButtonElement>) => void }) { return <div className="chat-read-status"><button type="button" aria-label={`Gelesen von ${readers.length} Personen`} aria-expanded={open} onClick={onToggle}>{readers.slice(0, 4).map((reader) => <span className="chat-reader-avatar" key={reader.id}>{reader.avatarUrl ? <img src={reader.avatarUrl} alt="" /> : initials(reader.name)}</span>)}{readers.length > 4 ? <span className="chat-reader-more">+{readers.length - 4}</span> : null}</button>{open ? <div className="chat-reader-popover"><strong>Gelesen von</strong>{readers.map((reader) => <span key={reader.id}>{reader.avatarUrl ? <img src={reader.avatarUrl} alt="" /> : <i>{initials(reader.name)}</i>}{reader.name}</span>)}</div> : null}</div>; }
