...
export default function ChatPage() {
  ...
  const [expandedPollOption, setExpandedPollOption] = useState<string | null>(null);
            ...
            {messages.map((message) => {
              ...
              return <div className={["chat-message-row", own ? "is-own" : "", swiping?.id === message.id ? "is-swiping" : ""].filter(Boolean).join(" ")} id={`chat-message-${message.id}`} key={message.id} onPointerDown={(event) => { swipeStartRef.current = { id: message.id, x: event.clientX, y: event.clientY }; }} onPointerMove={(event) => { const start = swipeStartRef.current; if (start?.id === message.id && event.clientX > start.x && Math.abs(event.clientY - start.y) < 45) setSwiping({ id: message.id, offset: Math.min(event.clientX - start.x, 92) }); }} onPointerUp={(event) => { const start = swipeStartRef.current; swipeStartRef.current = null; setSwiping(null); if (start?.id === message.id && event.clientX - start.x > 72 && Math.abs(event.clientY - start.y) < 45) { setReplyingTo(message); textareaRef.current?.focus(); } }} onPointerCancel={() => { swipeStartRef.current = null; setSwiping(null); }}>
                <article className={["chat-message", own ? "is-own" : "", emojiOnly ? "is-emoji-only" : "", message.poll ? "has-poll" : "", message.attachments.some((file) => file.type.startsWith("image/")) ? "has-image" : ""].filter(Boolean).join(" ")} style={{ transform: swiping?.id === message.id ? `translateX(${swiping.offset}px)` : undefined }} onContextMenu={(event) => { event.preventDefault(); event.stopPropagation(); setContextMenu({ message, x: Math.min(event.clientX, window.innerWidth - 230), y: Math.min(event.clientY, window.innerHeight - 330) }); }}>
                  ...
                  {message.poll ? <ChatPollCard message={message} sessionId={session?.id} profiles={profilesById} expandedOption={expandedPollOption} onToggleVoters={(optionId) => setExpandedPollOption((current) => current === optionId ? null : optionId)} onVote={(optionId) => void togglePollVote(message, optionId)} /> : message.body ? renderMessageBody(message.body, data.events, data.profiles) : null}
...
function ChatPollCard({ message, sessionId, profiles, expandedOption, onToggleVoters, onVote }: { message: ChatMessage; sessionId?: string; profiles: Map<string, Profile>; expandedOption: string | null; onToggleVoters: (optionId: string) => void; onVote: (optionId: string) => void }) { const poll = message.poll; if (!poll) return null; const totalVotes = poll.options.reduce((sum, option) => sum + option.voterIds.length, 0); const participantCount = new Set(poll.options.flatMap((option) => option.voterIds)).size; return <div className="chat-poll"><strong>{poll.question}</strong><div>{poll.options.map((option) => { const selected = Boolean(sessionId && option.voterIds.includes(sessionId)); const percent = totalVotes ? Math.round(option.voterIds.length / totalVotes * 100) : 0; return <div className="chat-poll-result" key={option.id}><button className={selected ? "is-selected" : ""} type="button" onClick={() => onVote(option.id)}><span className="chat-poll-fill" style={{ width: `${percent}%` }} /><span className="chat-poll-choice"><i>{selected ? "✓" : ""}</i><b>{option.label}</b><small>{option.voterIds.length}</small></span></button>{option.voterIds.length ? <button className="chat-poll-voters-toggle" type="button" aria-expanded={expandedOption === option.id} onClick={() => onToggleVoters(option.id)}>{expandedOption === option.id ? "Namen ausblenden" : "Wer?"}</button> : null}{expandedOption === option.id ? <div className="chat-poll-voter-list">{option.voterIds.map((id) => { const voter = profiles.get(id); return voter ? <span key={id}>{voter.avatarUrl ? <img src={voter.avatarUrl} alt="" /> : <i>{initials(voter.name)}</i>}{voter.name}</span> : null; })}</div> : null}</div>; })}</div><small>{participantCount} {participantCount === 1 ? "Stimme" : "Stimmen"}{poll.allowMultiple ? " · Mehrfachauswahl" : ""}</small></div>; }
